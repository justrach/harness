...
name = out.name
binary = out / 'profiled-harness'
shutil.copy2(args.binary, binary)
settings = json.loads(args.settings.read_text())
(out / 'ui-settings.json').write_text(json.dumps(settings))
env = os.environ | {'DISPLAY': args.display, 'WAYLAND_DISPLAY': '', 'HARNESS_IPC_PORT': args.ipc_port, 'HARNESS_PROVIDER': 'mock', 'HARNESS_CODEGRAFF_CLIENT_ID': '', 'HARNESS_EDGE_URL': 'http://127.0.0.1:1', 'HARNESS_DATA_DIR': str(out), 'RUST_LOG': 'warn', 'LP_NUM_THREADS': '4'}
...
try:
    time.sleep(2)
    ids = subprocess.check_output(['xdotool', 'search', '--class', 'harness'], env=env, text=True).splitlines()
    wid = ids[-1]
