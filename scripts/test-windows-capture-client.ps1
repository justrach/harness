...
public static class HarnessClientCapture {
...
[uint32]$owner = 0
[void][HarnessClientCapture]::GetWindowThreadProcessId($hwnd, [ref]$owner)
...
$previousDpi = [HarnessClientCapture]::SetThreadDpiAwarenessContext([IntPtr]::new(-4))
$bitmap = $null
try {
    $rect = [HarnessClientCapture+Rect]::new()
    if (-not [HarnessClientCapture]::GetClientRect($hwnd, [ref]$rect)) { throw 'GetClientRect failed' }
    $dpi = [HarnessClientCapture]::GetDpiForWindow($hwnd)
    ...
    try {
        ...
        if (-not [HarnessClientCapture]::PrintWindow($hwnd, $dc, 3)) { throw 'PrintWindow failed' }
    ...
    if ($previousDpi -ne [IntPtr]::Zero) { [void][HarnessClientCapture]::SetThreadDpiAwarenessContext($previousDpi) }
}
