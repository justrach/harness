#!/usr/bin/env python3
"""Replay text/reasoning from a real Harness Claude journal at a fixed cadence.

Set CLAUDE_CODE_EXECUTABLE to this file and HARNESS_REPLAY_JOURNAL to the JSONL
...
HARNESS_REPLAY_DELAY_MS defaults to 40; HARNESS_REPLAY_REPEAT defaults to 1.
...
with open(os.environ["HARNESS_REPLAY_JOURNAL"]) as source:
...
delay = float(os.environ.get("HARNESS_REPLAY_DELAY_MS", "40")) / 1000
repeats = int(os.environ.get("HARNESS_REPLAY_REPEAT", "1"))
if repeats < 1:
    raise ValueError("HARNESS_REPLAY_REPEAT must be positive")
