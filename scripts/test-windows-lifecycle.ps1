...
param(
    [string]$Exe = (Join-Path $PSScriptRoot '../target/release/harness.exe'),
    [int]$Runs = 2
)
...
if (-not ('HarnessWindowProbe' -as [type])) {
Add-Type @'
using System;
using System.Text;
using System.Runtime.InteropServices;
public static class HarnessWindowProbe {
...
try {
    ...
    foreach ($run in 1..$Runs) {
        ...
        try {
            ...
            [void][HarnessWindowProbe]::GetClassName($hwnd, $class, $class.Capacity)
            [uint32]$owner = 0
            [void][HarnessWindowProbe]::GetWindowThreadProcessId($hwnd, [ref]$owner)
            ...
            $device = (Get-Content -LiteralPath (Join-Path $root 'Harness/device-id') -Raw).Trim()
