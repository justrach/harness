...
// one API turn. RSS/CPU for the CLI child are reported separately from Harness.
...
const profiledBinary = `${output}/harness-profiled`;
...
if (process.env.HARNESS_PROFILE_UI_SETTINGS) {
  copyFileSync(resolve(process.env.HARNESS_PROFILE_UI_SETTINGS), `${output}/ui/ui-settings.json`);
}
const server = createServer();
await new Promise(r => server.listen(0, '127.0.0.1', r));
const port = server.address().port;
await new Promise(r => server.close(r));
const env = { ...process.env, HARNESS_IPC_PORT: String(port), HARNESS_DATA_DIR: `${output}/engine`,
  HARNESS_PROVIDER: harness, HARNESS_FRAME_STATS: process.env.HARNESS_FRAME_STATS ?? '1', RUST_LOG: 'warn',
  HARNESS_MOCK_CHARS: '24', HARNESS_MOCK_DELAY_MS: '20' };
delete env.HARNESS_EDGE_TOKEN;
delete env.HARNESS_ORG_ID;
const processes = [];
...
function stat(pid) {
  try {
    ...
    const pss = process.env.HARNESS_PROFILE_PSS === '1'
...
try {
  ...
  const backgroundChats = Number(process.env.HARNESS_PROFILE_BACKGROUND_CHATS ?? 0);
  ...
  const ui = start([`harness://open/chat/${chatId}?workspace=${locator}`], 'ui', { HARNESS_DATA_DIR: `${output}/ui` });
  ...
  if (!macOS && process.env.DISPLAY && !process.env.WAYLAND_DISPLAY) {
    await sleep(2000);
    const windows = execFileSync('xdotool', ['search', '--class', '^harness$'], { encoding: 'utf8' }).trim().split('\n');
    if (windows.length !== 1) throw Error('Use a dedicated X display with exactly one Harness window');
  ...
  phase = 'idle';
  await sleep(Number(process.env.HARNESS_PROFILE_PRE_IDLE_MS ?? 10000));
  if (ui.exitCode != null) throw Error('UI exited; inspect ui.log');
  phase = 'stream';
  const prompt = process.env.HARNESS_PROFILE_PROMPT ??
  ...
  const turns = Number(process.env.HARNESS_PROFILE_TURNS ?? 1);
  if (!Number.isSafeInteger(turns) || turns < 1) throw Error('HARNESS_PROFILE_TURNS must be a positive integer');
  for (let turn = 0; turn < turns; turn++) {
    const priorAssistants = new Set(transcript.filter(e => e.role === 'assistant').map(e => e.id));
    if (process.env.HARNESS_PROFILE_SUBMIT_UI === '1') {
    ...
    const timeoutMs = Number(process.env.HARNESS_PROFILE_TIMEOUT_MS ?? 240000);
  ...
  phase = 'settled';
  await sleep(Number(process.env.HARNESS_PROFILE_IDLE_MS ?? 15000));
  if (streamError) throw streamError;
  ...
  const summary = { binary, binarySha256, harness, mode: process.env.HARNESS_REPLAY_JOURNAL ? 'replay' : 'live',
    platform: process.platform, arch: process.arch,
    renderThreads: process.env.LP_NUM_THREADS ?? 'default', frameStats: env.HARNESS_FRAME_STATS,
    submission: process.env.HARNESS_PROFILE_SUBMIT_UI === '1' ? 'composer' : 'rpc', turns, backgroundChats, prompt,
  ...
  for (const name of ['engine', 'ui']) {
    const budget = process.env[`HARNESS_MAX_${name.toUpperCase()}_RSS_MIB`];
  ...
  if (process.env.HARNESS_PROFILE_HOLD_MS) await sleep(Number(process.env.HARNESS_PROFILE_HOLD_MS));
} finally {
  clearInterval(sampler);
