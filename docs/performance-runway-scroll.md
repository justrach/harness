...
Build each revision with `cargo build --release --locked -p harness` and copy the
...
DISPLAY=:108 WAYLAND_DISPLAY= LP_NUM_THREADS=4 HARNESS_FRAME_STATS=0 \
  HARNESS_PROFILE_PSS=1 HARNESS_PROFILE_SUBMIT_UI=1 HARNESS_PROFILE_IDLE_MS=45000 \
  CLAUDE_CODE_EXECUTABLE="$PWD/scripts/replay-claude.py" \
  HARNESS_REPLAY_JOURNAL="$PWD/scripts/fixtures/resource-stream.jsonl" \
  node scripts/resource-profile.mjs /path/to/harness /tmp/fresh-run claude-code
```

For the short workload, use `scripts/fixtures/runway-short-stream.jsonl`, set
`HARNESS_REPLAY_DELAY_MS=400`, `HARNESS_PROFILE_PRE_IDLE_MS=5000` and
`HARNESS_PROFILE_IDLE_MS=20000`. The replay makes no model API calls.
