...
Build with `cargo build --release --locked -p harness`, and copy each binary to an
...
HARNESS_PROFILE_BACKGROUND_CHATS=50 HARNESS_PROFILE_SUBMIT_UI=1 \
HARNESS_PROFILE_PROMPT='Replay fixture.' \
node scripts/resource-profile.mjs /path/to/harness /tmp/fresh-profile claude-code
```

The fixture emits 52,624 combined text/reasoning bytes with a 40 ms delta delay.
For faster streaming, set `HARNESS_REPLAY_REPEAT=4 HARNESS_REPLAY_DELAY_MS=10`.
For the crash regression, select `runway-short-stream.jsonl`, set the delay to
400 ms and `HARNESS_PROFILE_TURNS=8`. Submission uses actual native composer key
