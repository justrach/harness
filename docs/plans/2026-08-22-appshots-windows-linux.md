...
2. Add `crates/ui/src/appshots/windows.rs` using `RegisterHotKey`,
   ...
   to screenshot-only or a concrete capture error; Harness does not request
   `UIAccess`.
...
4. Add a Linux-only `harness appshot` activation command and local activation
...
- `cargo fmt --all -- --check`, `cargo test -p harness-ui --lib`,
  `cargo check -p harness-ui`, and `cargo check -p harness` on macOS.
