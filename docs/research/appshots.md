...
Scope: desktop Harness on macOS, Linux X11, and Linux Wayland

## Summary

An Appshot is a user-triggered capture of the frontmost application window. A
global keyboard shortcut works while Harness is in the background, captures both
the visible window and machine-readable application context, then stages that
capture in a Harness composer for review. It is not an attachment-menu action and
...
2. Harness captures the frontmost window before activating itself.
3. Harness opens the chosen composer and stages a screenshot plus accessibility
   context.
...
to Harness, attach a file, and manually copy otherwise invisible text.
...
- Harness never steals focus until the screenshot has been acquired.
...
rule: only reuse an existing composer if Harness had an active session selected
...
- Restore or focus the Harness window after capture.
...
- After successful staging, play a soft confirmation cue. Honor the
  dedicated capture sound setting and `HARNESS_DISABLE_SOUND`; failed captures stay silent.

## Architecture

### Placement

Capture belongs to the headed UI side of Harness, not the engine:
...
is kept correct in development by launching a separately signed `Harness Dev.app`
...
3. Emit a completed capture to Harness.
4. Resolve the destination.
5. activate/reveal Harness and stage the result.

Activating Harness before steps 1-2 would capture Harness itself.
...
| Harness window cannot be restored | Preserve capture in an inbox-like pending slot and notify. |
...
If Harness already persists composer drafts, Appshot metadata and the screenshot
...
- Capture frontmost window pixels without focusing Harness.
...
- Windows is outside this contribution: Harness does not yet ship a Windows app.
...
- Harness is not selected as the capture target due to activation ordering.
...
This runs after pixels are captured so Harness cannot replace the source image.
