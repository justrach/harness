...
actions for both Harness and the conversation's actual harness.
...
```text
ConversationLinks
  harness: required internal deeplink
...
The Harness URI must include an opaque workspace/profile locator in addition to
the durable chat id so local, synced, and development profiles cannot collide.
Proposed shape:

```text
harness://open/chat/<percent-encoded-chat-id>?workspace=<opaque-locator>
```

“Copy Harness deeplink” is complete only when the URI round-trips into the correct
...
2. Add a workspace-scoped Harness URI builder/parser and inbound route intent.
3. Register and handle `harness://` in macOS packaging first; add Linux desktop
