...
Reproduce with `cargo run -p harness-ui --example project-selector-fixture` on a desktop (or Xvfb with a window manager). Open **All projects**, then type `anara`. Down and Return select the second matching project; the trigger shows `anara @ Build server` and its offline glyph.
