...
labels, while retaining Harness's Geist fonts and desktop palette. Code line
...
xcodebuild -project apps/ios/Harness.xcodeproj -scheme Harness \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' test
```

`HarnessTests/TranscriptFollowTests` checks follow state independently of layout.
`HarnessTests/TranscriptLayoutTests` mounts the real SwiftUI transcript in a
...
`HarnessUITests/MobilePolishTests` exercises actual project selection/dismissal,
