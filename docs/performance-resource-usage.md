...
pins [`upstream Zui` at `49f9abd19`](https://github.com/zeronsh/zui/commit/49f9abd196705eda6a27080c506295a19b5da63e).
...
from old Harness**, and **33.3% from the previous optimized build**.
...
| Metric | Old Harness | Previous optimized | Final |
| --- | ---: | ---: | ---: |
...
cache. Final idle RSS remains 37.7% below old Harness.
...
the driver requires exactly one Harness window.

```sh
cargo build --release -p harness
Xvfb :98 -screen 0 1440x900x24 -nolisten tcp
# In another terminal:
DISPLAY=:98 WAYLAND_DISPLAY= LP_NUM_THREADS=4 HARNESS_FRAME_STATS=0 \
  HARNESS_PROFILE_PSS=1 HARNESS_PROFILE_IDLE_MS=45000 \
  CLAUDE_CODE_EXECUTABLE="$PWD/scripts/replay-claude.py" \
  HARNESS_REPLAY_JOURNAL="$PWD/scripts/fixtures/resource-stream.jsonl" \
  node scripts/resource-profile.mjs target/release/harness /tmp/harness-short claude-code

# Use LP_NUM_THREADS=1 for the one-worker check.
# Add HARNESS_REPLAY_REPEAT=10 HARNESS_REPLAY_DELAY_MS=8 for the long workload.
# For a live turn, omit replay variables and point CLAUDE_CODE_EXECUTABLE to
# the authenticated CLI; HARNESS_PROFILE_SUBMIT_UI=1 submits through the composer.
