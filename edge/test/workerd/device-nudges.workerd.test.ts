...
it("replays unacknowledged frames and retires only the matching token", async () => {
  ...
  const headers = { "x-harness-auth-user": "owner", upgrade: "websocket" };
  ...
  const post = () => room.fetch("https://test/nudge", { method: "POST", headers: { "x-harness-auth-user": "owner" }, body: JSON.stringify({chatId:"chat"}) });
...
it("drains 300 queued wakes through a bounded acknowledgment window", async () => {
  const room = env.DEVICE_ROOMS.get(env.DEVICE_ROOMS.idFromName(crypto.randomUUID()));
  const response = await room.fetch("https://test/ws?role=host&nudgeAck=1", { headers: { "x-harness-auth-user": "owner", upgrade: "websocket" } });
  const ws = response.webSocket!; ws.accept(); ws.binaryType = "arraybuffer";
...
it("keeps legacy hosts usable without acknowledgment support", async () => {
  const room = env.DEVICE_ROOMS.get(env.DEVICE_ROOMS.idFromName(crypto.randomUUID()));
  const response = await room.fetch("https://test/ws?role=host", { headers: { "x-harness-auth-user": "owner", upgrade: "websocket" } });
  const ws = response.webSocket!; ws.accept(); ws.binaryType = "arraybuffer";
  ...
  await room.fetch("https://test/nudge", {method:"POST",headers:{"x-harness-auth-user":"owner"},body:JSON.stringify({chatId:"legacy"})});
