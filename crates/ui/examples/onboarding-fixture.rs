    ...
    gpui_platform::application().with_assets(icons::Assets).run(move |cx| {
        ...
        cx.spawn(async move |cx| {
            let run: anyhow::Result<()> = async {
                ...
                window.update(cx, |shell, window, cx| shell.fixture_onboarding_split(true, window, cx))?;
                pause(cx, 1200).await;
                capture(window.into(), cx, &output, "03-split")?;
                window.update(cx, |shell, window, cx| shell.fixture_onboarding_split(false, window, cx))?;
                pause(cx, 800).await;

                window.update(cx, |shell, _, cx| shell.fixture_onboarding_starter("launch", cx))?;
                pause(cx, 1000).await;
                capture(window.into(), cx, &output, "04-starter-launch")?;

                state.update(cx, |state, cx| state.select_chat(Some("onboarding-chat".into()), cx));
                pause(cx, 1000).await;
                ...
                window.update(cx, |shell, window, cx| shell.fixture_command_palette(None, window, cx))?;
                pause(cx, 1000).await;
                ...
                window.update(cx, |shell, window, cx| shell.fixture_chat_tab(Some(0), window, cx))?;
                pause(cx, 800).await;
