    ...
    gpui_platform::application().with_assets(icons::Assets).run(move |cx| {
        ...
        cx.spawn(async move |cx| {
            let run:anyhow::Result<()>=async {
                pause(cx,1000).await;
                state.update(cx,|s,cx| {s.receive_transcript_frame(harness_doc::TranscriptFrame::Reset {reset:serde_json::from_value(serde_json::json!([
                ...
                state.update(cx,|s,cx|{s.queue.clear();s.receive_transcript_frame(harness_doc::TranscriptFrame::Reset{reset:serde_json::from_value(serde_json::json!([
