...
pub fn app_menus() -> Vec<Menu> {
    ...
    let mut app_items = vec![
        ...
        MenuItem::action("About Harnesser", About).disabled(!macos),
        MenuItem::separator(),
    ...
    if macos {
        app_items.extend([
            ...
            MenuItem::separator(),
            MenuItem::action("Hide Harnesser", Hide),
            MenuItem::action("Hide Others", HideOthers),
            MenuItem::action("Show All", ShowAll),
            MenuItem::separator(),
        ]);
    }
    app_items.push(MenuItem::action("Quit Harnesser", Quit));

    let mut menus = vec![
        Menu::new("Harnesser").items(app_items),
...
// (`cargo run`) show "Harnesser" and the crate version instead of the process name.
#[cfg(target_os = "macos")]
mod about_panel {
    ...
    pub(super) fn show() {
        ...
        let name = NSString::from_str("Harnesser");
