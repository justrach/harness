...
//! The rows live on the session doc ([`harness_doc::QueuedMessage`]), so the phone
...
use harness_doc::{QueueDeliveryGate, QueuedMessage};
...
impl Composer {
    ...
    pub(crate) fn render_queue_panel(
        ...
        let (items, chat_id, host_supports_actions) = {
            ...
            let host_supports_actions = state.chat_host_supports(
                &chat_id,
                harness_proto::capabilities::MESSAGE_QUEUE_ACTIONS_V1,
            );
    ...
    pub(crate) fn begin_queue_edit(&mut self, id: String, cx: &mut Context<Self>) {
        ...
        let (chat_id, host_device_id, supported) = {
            ...
            let capability = harness_proto::capabilities::MESSAGE_QUEUE_EDIT_LEASE_V1;
...
mod tests {
    ...
    fn queue_shortcut_targets_the_most_recently_added_row() {
        let items = vec![
            harness_doc::QueuedMessage::new("older", "first", "device"),
            harness_doc::QueuedMessage::new("newer", "second", "device"),
        ];
