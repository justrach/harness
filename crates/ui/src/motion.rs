//! Animation kit — the harness motion catalog as reusable helpers over gpui
...
//! - `harness-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// harness's signature entrance curve — CSS `cubic-bezier(0.16, 1, 0.3, 1)`.
...
/// (`transition-colors` et al. carry it unless overridden; harness never does).
...
/// Harness loader pulse period: 2.4s.
pub const HARNESS_PULSE: MotionSpec = MotionSpec::new(2400, EASE);
...
/// (harness also scales 0.96→1; divs have no scale transform in gpui — approximated.)
...
// the pointer enters. The original harness puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`HARNESS_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `HARNESS_MOTION_SCALE=10` slows the
...
pub fn speed_scale() -> f32 {
    ...
    *SCALE.get_or_init(|| {
        std::env::var("HARNESS_MOTION_SCALE")
            .ok()
...
mod tests {
    ...
    #[test]
    fn catalog_timings_match_harness() {
        ...
        assert_eq!(HARNESS_PULSE.duration_ms, 2400);
