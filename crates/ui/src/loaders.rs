//! Loaders: the harness pulse loader, the gradient matrix spinner, and the boot
        ...
        .children(MARK_CELLS.iter().map(move |&(x, y)| {
            ...
            div()
                ...
                .child({
                    ...
                    // the stagger ADDS phase (harness-loader.tsx delayFor).
...
pub fn harness_loader(
    ...
    div()
        ...
        .items_center()
        .gap(px(slot / 2.0))
        .children((0..HARNESS_CELLS).map(move |i| {
...
/// The gradient matrix spinner (WorkingIndicator), ported from harness's
...
const _: () = {
    ...
    assert!(HARNESS_PULSE.duration_ms == 2400);
