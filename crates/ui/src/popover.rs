...
use crate::motion::{self, HARNESS_PULSE};
...
/// The floating-menu surface (harness `.glass-surface` + `menuSurface`):
...
/// One menu row (harness `menuItem`): `gap-2.5 rounded-lg px-2 py-1.5
...
/// `bg-white/[0.08]` (harness's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (harness
...
/// Hairline divider between menu sections (harness `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (harness
...
/// A bordered trailing menu section (harness picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (harness dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (harness dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (harness:
...
pub fn skeleton_rows(
    ...
    let wash = ink(0.04);
    let delta = motion::pulse_delta(&HARNESS_PULSE, view, cx);
    div()
        .flex()
        .flex_col()
        .gap(px(6.0))
