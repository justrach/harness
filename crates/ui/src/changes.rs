...
use harness_proto::{Chat, CheckoutDiff, GitHistoryCommit};
...
pub struct DiffHighlights {
    pub old: Option<Arc<harness_syntax::HighlightedDocument>>,
    pub new: Option<Arc<harness_syntax::HighlightedDocument>>,
}
...
impl DiffHighlights {
    ...
    pub fn spans(&self, line: &DiffLine) -> &[harness_syntax::HighlightSpan] {
...
fn excerpt_side(
    ...
    Some(Arc::new(harness_syntax::HighlightedDocument {
        language,
        lines,
    }))
}
...
fn excerpt_highlights(file: &FileDiff, language: Lang) -> Option<DiffHighlights> {
    if !harness_syntax::supports_language(language) {
        return None;
    }
...
fn sources_match_patch(file: &FileDiff, response: &harness_proto::CheckoutFileDiffText) -> bool {
    let old = response
        .old_text
        .as_deref()
...
fn full_highlights(
    file: &FileDiff,
    language: Lang,
    response: &harness_proto::CheckoutFileDiffText,
    ...
    if old.is_none() && new.is_none() && harness_syntax::supports_language(language) {
        return None;
    }
    Some(DiffHighlights { old, new })
}
...
impl Changes {
    ...
    fn request_highlight(
        ...
        if !harness_syntax::supports_language(lang) {
        ...
        let fetch_task = match (active, engine) {
            (Some(diff), Some(engine)) => Some(cx.spawn(async move |this, cx| {
                let request = harness_proto::GetCheckoutFileDiffTextRequest {
                ...
                let response = engine
                    ...
                    .ok()
                    .and_then(|value| {
                        serde_json::from_value::<harness_proto::CheckoutFileDiffText>(value).ok()
                    });
...
fn diff_line_row(
    line: &DiffLine,
    spans: &[harness_syntax::HighlightSpan],
    theme: &Theme,
    gutter_px: f32,
        ...
        let scoped_notice: Option<(SharedString, bool)> = (!no_chat
            ...
            .map(|message| {
                if message.contains("no turn recorded") {
                    ...
                    (
                        SharedString::from(
                            "This chat's device is running an older Harness — update it to view branch and turn diffs",
                        ),
                        false,
                    )
                } else {
...
";
    ...
    fn sticky_header_uses_the_content_theme_in_dark_and_light() {
        use harness_theme::{AccentSelection, SurfacePreference};
    ...
    fn phases() {
        ...
        summarized.files.push(harness_proto::DiffFileSummary {
            path: "x".into(),
            old_path: None,
    ...
    fn full_diff_highlights_map_old_new_and_context_by_source_line() {
        ...
        assert!(
            highlights
                .spans(&deleted)
                .iter()
                .any(|span| span.kind == harness_syntax::HighlightKind::Function)
        );
        assert!(
            highlights
                .spans(&added)
    ...
    fn split_line_runs_use_affected_old_and_new_documents() {
        ...
        for (path, source, required) in [
            (
                ...
                harness_syntax::HighlightKind::Tag,
            ),
            (
                "src/Greeter.kt",
                "fun greet(name: String) = println(name)",
                harness_syntax::HighlightKind::Function,
            ),
            (
                "Dockerfile",
                "RUN echo \"hello\"",
    ...
    fn excerpt_parses_old_and_new_hunks_as_separate_documents() {
        ...
        assert!(
            highlights
                .spans(deleted)
                .iter()
                .any(|span| span.kind == harness_syntax::HighlightKind::Comment)
        );
        assert!(
            highlights
                .spans(added)
