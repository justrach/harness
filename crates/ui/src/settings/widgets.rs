...
//! (harness settings.devices.tsx / settings.agents.tsx / settings.archived.tsx).
...
/// title + `text-[13px]` count sharing a baseline (harness settings.devices.tsx).
...
/// Display-only toggle switch (harness branch-picker.tsx `Toggle`): an 18×32
