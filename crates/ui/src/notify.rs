...
//!   the banner is attributed to Harness with its icon. The API is deprecated
...
//!   center — those adopt the installed Harness app's bundle identity (the
//!   terminal-notifier technique: override `-[NSBundle bundleIdentifier]` for
//!   the main bundle), so dev banners still show the Harness icon; a click may
//!   focus the installed app rather than the dev binary — acceptable for a
//!   dev-only path. Machines without Harness installed fall back to
...
/// `-[NSBundle bundleIdentifier]` to answer the installed Harness app's id for
...
fn post_impl(title: &str, body: &str, _chat_id: Option<&str>) {
    ...
    std::thread::spawn(move || {
        ...
        let result = std::process::Command::new("notify-send")
            .args(["--app-name=Harness", "--", &title, &body])
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null())
            .status();
        match result {
