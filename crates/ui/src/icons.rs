...
//!
//! The set mirrors the original harness's icon usage exactly:
...
//!   are ports of the hand-drawn inline SVGs in harness's `terminal-panel.tsx` /
//!   `composer-actions.tsx`.
//! - The harness brand marks (`claude-mark`, `openai-mark`, `cursor-mark`) are
//!   ports of harness's `icons.tsx`. gpui tints SVGs with the text color, so the
...
icon_assets![
    ...
    // Mirrored variant (harness window-controls.tsx `-scale-x-100`): the LEFT
    ...
    // Hand-drawn harness glyphs (terminal-panel.tsx / composer-actions.tsx /
...
/// The Claude mark's brand orange (`#D97757`) — harness keeps it even on the
/// monochrome surface.
