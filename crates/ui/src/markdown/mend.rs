...
//! `remend`, ported to harness's incremental parser).
//!
...
//! handlers don't apply here — harness renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "harness:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](harness:pending-link)");
        mends("[docs](", "[docs](harness:pending-link)");
        mends("see [do", "see [do](harness:pending-link)");
        mends("![alt](https://x/i.p", "![alt](harness:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](harness:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](harness:pending-link)");
        mends("**a [b", "**a [b](harness:pending-link)**");
    }
