...
/// Gap between markdown blocks inside one message (harness mdBlockGap).
pub const MD_BLOCK_GAP: f32 = 12.0;
/// Body text size / line height (harness: 14px / 22px).
...
// Table metrics — a port of mugen-markdown 0.6.2's `TableBlock` under harness's
...
/// Uniform cell padding in px (harness `table.cellPadding`).
pub const TABLE_CELL_PADDING: f32 = 12.0;
/// Hairline between rows in px (harness `table.gap`).
pub const TABLE_DIVIDER: f32 = 1.0;
/// Header row font weight (harness `table.headerWeight` = 700).
...
/// Minimum rendered column width in px, padding included (harness
...
/// Hairline tone (harness md theme `table.borderColor`: rgba(255,255,255,0.1)).
...
/// Tight monochrome heading scale (harness: h2 ≈ 16px semibold; headings step
...
/// A GFM table — a port of mugen-markdown's `TableBlock` under harness's md
...
/// harness's `table.headerWeight`; strong runs never drop below semibold).
fn flatten_runs_weighted(runs: &[InlineRun], theme: &Theme, base_weight: FontWeight) -> FlatText {
    ...
    for run in runs {
        ...
        // Links stay monochrome — foreground with an underline (harness's md
