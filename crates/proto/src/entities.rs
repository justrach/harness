...
//!
//! In harness these were synced Postgres rows; in harness they live in the per-org
...
pub struct Device {
    ...
    /// First registration time (harness devices.created_at — the Devices page
...
pub struct Chat {
    ...
    /// continuity across engine restarts (harness's `chats.harness_session_id`).
    ...
    /// The chat whose agent created this one (via the Harness MCP server):
...
pub struct Worktree {
    ...
    pub path: String,
    pub branch: String,
    /// Generated worktree folder name (`harness/<name>` is its branch).
    #[serde(default)]
    pub name: String,
