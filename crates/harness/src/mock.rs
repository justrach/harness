...
/// The scripted question set for the `HARNESS_MOCK_QUESTION` variant (exercises
...
impl Harness for MockHarness {
    ...
    async fn run(
        ...
        // Optional pacing knob for demos/manual testing: `HARNESS_MOCK_DELAY_MS`
        ...
        let delay_ms = std::env::var("HARNESS_MOCK_DELAY_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(0);
        let delay = std::time::Duration::from_millis(delay_ms);

        // Dev/testing knob: `HARNESS_MOCK_QUESTION=1` swaps in a run that asks
        ...
        let question_mode = std::env::var("HARNESS_MOCK_QUESTION")
            .ok()
        ...
        // Dev/testing knob: `HARNESS_MOCK_REPEAT=N` loops the script body N times
        ...
        // at the very end).
        let repeat = std::env::var("HARNESS_MOCK_REPEAT")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or(1)
            .max(1);
        // Dev/testing knob: `HARNESS_MOCK_ERROR=1` appends a scripted error
        // before the terminal Done — the only data-side way to put the
        // transcript ErrorChip on screen with the mock harness.
        let mock_error = std::env::var("HARNESS_MOCK_ERROR")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        // Dev/testing knob: `HARNESS_MOCK_TABLE=1` appends scripted GFM tables
        ...
        let mock_table = std::env::var("HARNESS_MOCK_TABLE")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        let done_ix = self
            .script
        ...
        // Dev/testing knob: `HARNESS_MOCK_CODE=1` appends rust + ts code blocks
        ...
        let mock_code = std::env::var("HARNESS_MOCK_CODE")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        ...
        // Dev/testing knob: `HARNESS_MOCK_MEND=1` appends a link/list-heavy
        ...
        let mock_mend = std::env::var("HARNESS_MOCK_MEND")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        ...
        // Dev/testing knob: `HARNESS_MOCK_THINKING=1` appends a markdown-heavy
        ...
        let mock_thinking = std::env::var("HARNESS_MOCK_THINKING")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        let thinking_events = mock_thinking
            ...
            .into_iter()
            .flatten();
        // Dev/testing knob: `HARNESS_MOCK_SUBAGENT=1` appends two spawn chips
        ...
        // paced run (`HARNESS_MOCK_DELAY_MS`) holds a Running chip long enough
        // to observe.
        let mock_subagent = std::env::var("HARNESS_MOCK_SUBAGENT")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        ...
        let events: Vec<Result<AgentEvent, HarnessError>> = body
            ...
            .map(Ok)
            .collect();
        // Dev/testing knob: `HARNESS_MOCK_CHARS=N` re-chunks every TextDelta
        // into N-char deltas, so `HARNESS_MOCK_DELAY_MS` paces *characters*
        ...
        let chunk_chars = std::env::var("HARNESS_MOCK_CHARS")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&n| n > 0);
        ...
        // Dev/testing knob: `HARNESS_MOCK_SUBAGENT_DELAY_MS` paces TAGGED
        // (subagent) events on their own clock — the parent turn settles at
        // `HARNESS_MOCK_DELAY_MS` speed while the background subagents stream
        ...
        let sub_delay_ms = std::env::var("HARNESS_MOCK_SUBAGENT_DELAY_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok());
