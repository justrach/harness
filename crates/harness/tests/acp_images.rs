...
//! text when `initialize` advertises `promptCapabilities.image`, and are
//! left as path refs in the text otherwise.
...
async fn sent_prompt(images: bool) -> serde_json::Value {
    ...
    let request = RunRequest {
        ...
        auto_approve: true,
        attachments: vec![image.to_str().unwrap().into(), "/nonexistent/missing.png".into()],
        worktree: None,
        resume: None,
    };
    tokio::time::timeout(Duration::from_secs(10), async {
        harness.run(request, controls).await.unwrap().collect::<Vec<_>>().await
    })
    .await
    .unwrap();
...
async fn images_ride_the_first_prompt_only_when_the_agent_takes_them() {
    ...
    assert_eq!(blocks.len(), 2, "one image; the unreadable path is skipped: {with}");
