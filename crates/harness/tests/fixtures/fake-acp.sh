#!/bin/sh
# Fake ACP agent for harness-adapters tests.
#
...
# Grok attaches CLI login on every stdio spawn before session/new.
read -r line || exit 1
if has "$line" '"method":"authenticate"'; then
  has "$line" '"methodId":"grok.com"' || exit 1
...
*scenario:happy*)
  ...
  update '{"sessionUpdate":"tool_call","toolCallId":"t1","title":"cargo test -p harness-adapters","kind":"execute","status":"pending","rawInput":{"command":"cargo test -p harness-adapters"}}'
  update '{"sessionUpdate":"tool_call_update","toolCallId":"t1","status":"completed","content":[{"type":"content","content":{"type":"text","text":"   Compiling harness-adapters v0.1.21\n    Finished `dev` profile [unoptimized] in 2.41s\n     Running tests/acp.rs\n\nrunning 13 tests\ntest result: ok. 13 passed; 0 failed; 0 ignored"}}]}'
