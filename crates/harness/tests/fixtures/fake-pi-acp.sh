...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        result = {"sessionId":"pi-session", "configOptions":[{"id":"model","category":"model","type":"select","currentValue":"mock/old","options":[{"value":"mock/model","name":"Mock"}]},{"id":"thought_level","category":"thought_level","type":"select","currentValue":"off","options":[{"value":"xhigh","name":"Extra high"}]}]}
        ...
        if text == "tree":
            child = subprocess.Popen(["sh", "-c", "trap \"\" TERM; sleep 300 & wait"])
