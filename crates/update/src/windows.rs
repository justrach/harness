...
const BACKUP: &str = "harness.exe.old";
...
pub(super) fn is_managed(exe: &Path) -> bool {
    exe.file_name().is_some_and(|name| name == "harness.exe")
...
pub fn artifact(version: &str) -> String {
    format!("harness-{version}-windows-{}.exe", std::env::consts::ARCH)
}
...
pub async fn stage(
    ...
    let staged = temporary.path().join("harness.exe");
    ...
    .context("staged executable version check timed out")??;
        ...
        output.status.success()
            && String::from_utf8_lossy(&output.stdout).trim()
                == format!("harness {}", manifest.version),
...
pub fn apply(staged: &Path, directory: &Path, relaunch: bool) -> anyhow::Result<()> {
    let installed = directory.join("harness.exe");
    ensure!(
...
mod tests {
    ...
    /// A staged update layout: `<dir>/harness.exe` plus its `sha256` sidecar.
    fn staged_with(dir: &Path, bytes: &[u8]) -> PathBuf {
        let staged = dir.join("harness.exe");
    ...
    fn portable_install_requires_explicit_configuration() {
        ...
        let exe = dir.path().join("harness.exe");
    ...
    fn failed_swap_after_old_image_moved_restores_the_installation() {
        ...
        let installed = install.path().join("harness.exe");
    ...
    async fn corrupt_download_preserves_install_and_removes_staging() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let installed = dir.path().join("harness.exe");
