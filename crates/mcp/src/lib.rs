...
//!
//! `harness mcp` speaks MCP (JSON-RPC 2.0, newline-delimited) on stdin/stdout
//! and proxies every tool into the engine's localhost IPC — the same
//! `harness_rpc` WebSocket the headed app and `harness sync` dial. Nothing here
...
//! agent's MCP config with `HARNESS_CHAT_ID` / `HARNESS_DEVICE_ID` in the
...
pub use harness::{Origin, Harness};

/// How `harness mcp` finds the engine and who it speaks for.
#[derive(Debug, Clone)]
pub struct McpConfig {
    /// Loopback IPC port of the engine to proxy (`HARNESS_IPC_PORT`, default 27664).
    pub ipc_port: u16,
...
impl McpConfig {
    /// Resolve from the process environment: `HARNESS_IPC_PORT` for the engine,
    /// `HARNESS_CHAT_ID` / `HARNESS_DEVICE_ID` for the originating chat.
    pub fn from_env() -> Self {
        let ipc_port = std::env::var("HARNESS_IPC_PORT")
            .ok()
...
pub async fn run(config: McpConfig) -> anyhow::Result<()> {
    let harness = Harness::new(format!("ws://127.0.0.1:{}", config.ipc_port), config.origin);
    let tools = Tools::new(std::sync::Arc::new(harness));
