...
/// harness. Unset when a human runs `harness mcp` from a terminal.
...
impl Origin {
    pub fn from_env() -> Self {
        ...
        Self {
            chat_id: read("HARNESS_CHAT_ID"),
            device_id: read("HARNESS_DEVICE_ID"),
        }
    }
}
...
impl Harness {
    /// Lazy dialer: nothing connects until the first tool call, so `harness
