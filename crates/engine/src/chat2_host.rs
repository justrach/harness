...
pub struct EdgeCheckpointFetcher {
    priority: harness_sync::budget::Priority,
    http: reqwest::Client,
    edge: EdgeConfig,
    chat_id: String,
}
...
impl EdgeCheckpointFetcher {
    pub fn with_priority(mut self, priority: harness_sync::budget::Priority) -> Self {
    ...
    pub fn new(http: reqwest::Client, edge: EdgeConfig, chat_id: impl Into<String>) -> Self {
        Self {
            priority: harness_sync::budget::Priority::Interactive,
            http,
            edge,
...
impl CheckpointFetcher for EdgeCheckpointFetcher {
    fn fetch(&self) -> BoxFuture<'static, Result<Vec<u8>, SyncError>> {
        ...
        Box::pin(async move {
            ...
            for _attempt in 0..4 {
                let _permit = harness_sync::budget::shared().http(priority).await?;
