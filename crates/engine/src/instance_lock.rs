...
impl InstanceLock {
                ...
                match errno.raw_os_error() {
                    ...
                    Some(libc::EWOULDBLOCK) => {
                        ...
                        return Err(EngineError::Other(format!(
                            "another Harness engine is already running on {} (pid {}); \
                             stop it or use a different data dir (HARNESS_DATA_DIR)",
                            data_dir.display(),
    ...
    /// cannot test a lock without taking it). Used by `harness status` and the
...
mod tests {
    ...
    fn subprocess_lock_holder() {
        let Some(dir) = std::env::var_os("HARNESS_INSTANCE_LOCK_TEST_DIR") else {
            return;
        };
    ...
    fn subprocess_contention_reports_pid_and_releases_on_exit() {
        ...
        let mut child = Command::new(std::env::current_exe().expect("current test executable"))
            ...
            .arg("--nocapture")
            .env("HARNESS_INSTANCE_LOCK_TEST_DIR", dir.path())
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
