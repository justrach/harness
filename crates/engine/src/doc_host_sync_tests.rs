    ...
    for round in 0..3 {
        ...
        tokio::time::timeout(Duration::from_secs(20), async {
            loop {
                ...
                assert!(harness_sync::budget::shared().stats().sockets <= 32);
                assert_eq!(
...
async fn global_saturation_rotates_between_profiles_below_their_local_caps() {
    ...
    until(|| harness_sync::budget::shared().stats().socket_waiting > 0).await;
    ...
    until(|| {
        harness_sync::budget::shared().stats().sockets == 0
            && harness_sync::budget::shared().stats().socket_waiting == 0
    })
    .await;
}
