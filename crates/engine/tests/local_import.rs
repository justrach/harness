...
use harness_doc::{MessagePart, MessageRole, SessionDoc, SessionMessageEntry};

use harness_engine::local_import::{ImportEvent, marker_grants_read_root};
use harness_engine::run_journal::journal_paths;
use harness_engine::{EngineCore, EngineProfile, HarnessId, default_registry};
...
async fn seed_local(data_dir: &std::path::Path) -> (String, String, String) {
    ...
    let store = harness_sync::DocsStore::open(data_dir.join("profiles").join("local"))
...
async fn local_work_imports_into_synced_profile_once() {
    ...
    let store = harness_sync::DocsStore::open(dir.path().join("orgs").join("org1").join("user1"))
    ...
    let source_store = harness_sync::DocsStore::open(dir.path().join("profiles").join("local"))
