...
use harness_doc::{CommandBasedOn, SessionCommandEntry, SessionCommandPayload, SessionCommandStatus};
...
async fn bridge(
    a: &EngineCore,
    b: &EngineCore,
) -> harness_sync::registry::mock_server::MockRegistryServer {
    let server = harness_sync::registry::mock_server::MockRegistryServer::start().await;
...
async fn two_engines_share_a_workspace() {
    ...
    let client_a = harness_rpc::memory_client(a.rpc_service());
    let client_b = harness_rpc::memory_client(b.rpc_service());
    client_a
        .call(
            methods::MUTATE,
            serde_json::json!({
    ...
    wait_for(
        || {
            handle.doc().read_entries().unwrap_or_default().iter().any(|e| {
                e.parts.iter().any(
                    |p| matches!(p, harness_doc::MessagePart::Text { text, .. } if text == "From cursor"),
                )
            })
        },
...
async fn legacy_workspace_doc_migrates_instantly_on_first_boot() {
    use harness_proto::{Chat, Device, Session, Space};
    ...
    {
        let store = harness_sync::DocsStore::open(&org_dir).expect("open store");
        let legacy = harness_doc::WorkspaceDoc::new();
    ...
    let store = harness_sync::DocsStore::open(&org_dir).expect("reopen store");
    assert!(
        store
            .load_snapshot(harness_doc::REGISTRY_DOC_ID)
