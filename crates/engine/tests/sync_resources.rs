...
fn sync_keeps_git_available_with_256_file_descriptors() {
    let output = std::process::Command::new(std::env::current_exe().unwrap())
        ...
        .env("HARNESS_RESOURCE_TEST_CHILD", "1")
        .output()
        .unwrap();
...
fn resource_limit_child() {
    if std::env::var_os("HARNESS_RESOURCE_TEST_CHILD").is_none() {
        return;
    }
    unsafe {
...
async fn stress() {
    ...
    use harness_engine::{DocHost, DocHostConfig, EdgeConfig};
    ...
    tokio::time::timeout(Duration::from_secs(3), async {
        while harness_sync::budget::shared().stats().sockets != 0 {
