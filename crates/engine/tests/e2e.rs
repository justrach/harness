...
fn queue_as_viewer(doc: &SessionDoc, id: &str, payload: SessionCommandPayload) {
    ...
    let based_on =
        doc.read_entries()
            .expect("read entries")
            .last()
            .map(|m| harness_doc::CommandBasedOn {
...
async fn steer_with_no_live_run_falls_back_to_new_turn() {
    ...
    // dispatch-as-next-turn, per harness's executor.
    queue_as_viewer(
        handle.doc(),
        "cmd-steer-1",
...
async fn processed_commands_are_skipped_on_redelivery() {
    ...
    let verdict = harness_doc::evaluate_command(
        entry,
        &harness_doc::EvaluationContext {
    ...
    assert_eq!(verdict, harness_doc::CommandDisposition::Skip);
}
...
async fn rpc_surface_over_in_memory_transport() {
    ...
    let models = client
        .call(
            harness_rpc::methods::LIST_MODELS,
    ...
    let mut messages_stream = client
        .subscribe(
            harness_rpc::methods::WATCH_DOC_MESSAGES,
    ...
    let queued = client
        .call(
            harness_rpc::methods::QUEUE_COMMAND,
    ...
    let settled = loop {
        let item = tokio::time::timeout_at(deadline, messages_stream.recv())
            ...
            .expect("stream alive");
        let frame: harness_doc::TranscriptFrame = serde_json::from_value(item).unwrap();
        harness_doc::apply_transcript_frame(&mut materialized, frame).unwrap();
...
async fn respond_input_resolves_pending_question() {
    ...
    impl Harness for AskingHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let answers = (controls.request_input)(vec![harness_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
    ...
    queue_as_viewer(
        ...
        SessionCommandPayload::RespondInput {
            request_id,
            answers: vec![harness_proto::UserInputAnswer {
                question_id: "q1".into(),
                labels: vec!["b".into()],
            }],
        },
        ...
        async fn run(
            ...
            if request.prompt == "second run" {
                ...
                tokio::spawn(async move {
                    ...
                    let _ = (controls.request_input)(vec![harness_proto::UserInputQuestion {
                        id: "q1".into(),
                        header: "Pick".into(),
                        question: "Which one?".into(),
                        options: vec!["a".into(), "b".into()],
...
async fn harness_emitted_input_twin_is_dropped_and_answer_resumes() {
    ...
    impl Harness for DoubleEmitHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let question = harness_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
                    question: "Which one?".into(),
                    options: vec!["a".into(), "b".into()],
...
async fn attachment_upload_then_run_threads_refs_and_paths() {
    ...
    for (seq, data) in [(0, first), (1, second)] {
        client
            .call(
                harness_rpc::methods::UPLOAD_CHUNK,
    ...
    let committed = client
        .call(
            harness_rpc::methods::UPLOAD_COMMIT,
    ...
    // Run with the harness `withAttachments` transport: refs embedded in the
    ...
    let chunk = client
        .call(
            harness_rpc::methods::READ_ATTACHMENT_CHUNK,
...
/// Run with: `cargo test -p harness-engine --test e2e -- --ignored`
#[tokio::test(flavor = "multi_thread")]
        ...
        if routed_dispatch {
            ...
            assert_eq!(
                core.sessions
                    ...
                    .await
                    .unwrap(),
                harness_engine::sessions::SteerOutcome::Accepted
            );
        }
...
async fn generated_image_is_materialized_before_publication_and_survives_reopen() {
    use harness_engine::{DocHost, DocHostConfig, SessionsEngine, Uploads};
    let dir = tempfile::tempdir().unwrap();
    ...
    // Canonical on both sides: macOS tempdirs live under the /var ->
    // /private/var symlink, and materialized paths are canonical.
    assert!(
        std::path::Path::new(&images[0]).starts_with(uploads.dir().canonicalize().unwrap()),
        "{} not under {}",
        images[0],
        uploads.dir().display()
    );
...
async fn real_image_generation_profile_smoke() {
    ...
    let core = EngineCore::assemble(
        dir.path(),
        registry_with(Arc::new(harness_adapters::CodexHarness::new())),
        HarnessId::Codex,
        None,
    )
    .unwrap();
