...
//! beyond sqlite's own).
//!
//!   cargo run -p harness-engine --example doc_inspect -- <store-root> <chat-id>

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut args = std::env::args().skip(1);
    let root = args.next().expect("store root");
    let chat_id = args.next().expect("chat id");
    let store = harness_sync::DocsStore::open(std::path::PathBuf::from(root))?;
    ...
    raw.import(&bytes)?;
    let doc = harness_doc::SessionDoc::from_doc(raw);
