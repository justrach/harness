...
use harness_proto::{Chat, Device, Session, SessionStatus};
use harness_sync::registry::mock_server::MockRegistryServer;
use harness_sync::{DocsStore, RegistryClient, RegistryEvent};
...
impl harness_sync::registry::RegistryTransport for FlakyAckTransport {
    fn fetch(
        &self,
        _since: u64,
...
async fn unreadable_http_ack_retries_instead_of_stranding_the_batch() {
    ...
    let client = RegistryClient::connect_via_transport(
        Arc::new(harness_sync::StaticUrl("ws://127.0.0.1:1/ws".into())),
        doc.clone(),
        "dev-a",
        harness_sync::RegistryTuning::default(),
