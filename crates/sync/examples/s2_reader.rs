//! Fresh RUST reader for an s2 loro room — the exact client path every comet
...
//! Usage: cargo run -p comet-sync --example s2_reader -- <wsUrl> <roomId>
use loro::LoroDoc;
...
async fn main() {
    ...
    let doc = LoroDoc::new();
    let client = comet_sync::RoomClient::connect(url, room_id, doc.clone())
        .await
        .expect("connect");
