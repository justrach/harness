//! `harness update` — check for and apply a newer release, natively (the same
...
pub async fn update(edge_url: &str, check_only: bool) -> anyhow::Result<()> {
    ...
    if !version_newer(&manifest.version, current) {
        println!(
            "Harness {current} is up to date (latest: {}).",
            manifest.version
        );
        return Ok(());
    }
    println!("Harness {current} → {} available", manifest.version);
    if check_only {
        std::process::exit(1);
    }
