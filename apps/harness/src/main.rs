...
struct Cli {
    ...
    /// A folder to open as a project (defaults to the current directory when
    /// run from a terminal), or a Harness conversation URL.
    #[arg(value_name = "FOLDER|URL")]
...
#[derive(Subcommand)]
enum DaemonCommand {
    /// Install, enable, and start the service (captures HARNESS_* env).
    Install,
...
/// Production edge (Cloudflare Worker + Durable Objects at edge.codegraff.com).
/// `HARNESS_EDGE_URL` overrides (local dev / self-hosting).
const DEFAULT_EDGE_URL: &str = "https://edge.codegraff.com";

/// Public CodeGraff OAuth client id. Overridden by `HARNESS_CODEGRAFF_CLIENT_ID`;
/// set it to the empty string — or set a dev bearer via `HARNESS_EDGE_TOKEN` — to
/// force dev-mode auth instead.
const DEFAULT_CODEGRAFF_CLIENT_ID: &str = "cg_client_43e753878956c2cf7b5b0f53";

fn edge_url_from_env() -> String {
    paths::var("HARNESS_EDGE_URL")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| DEFAULT_EDGE_URL.into())
}

/// CodeGraff client id resolution: explicit env wins (empty string = dev mode);
/// otherwise a `HARNESS_EDGE_TOKEN` dev bearer keeps dev mode (smoke tests,
...
fn codegraff_client_id_from_env(edge_token: &Option<String>) -> Option<String> {
    match paths::var("HARNESS_CODEGRAFF_CLIENT_ID") {
        Ok(v) if v.trim().is_empty() => None,
        Ok(v) => Some(v),
        Err(_) if edge_token.is_some() => None,
        Err(_) => Some(DEFAULT_CODEGRAFF_CLIENT_ID.into()),
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        None => {
            let initial_url = match launch::resolve_open_arg(cli.open_url, launch::from_terminal())
            {
                Ok(url) => url,
                Err(err) => {
            ...
            harness_ui::run_app(harness_ui::UiConfig {
                ...
                ipc_port: paths::var("HARNESS_IPC_PORT")
                    .ok()
                    .and_then(|p| p.parse().ok())
                    .unwrap_or(27664),
                edge_url: edge_url_from_env(),
                codegraff_client_id: codegraff_client_id_from_env(&edge_token),
                edge_token,
                org_id: paths::var("HARNESS_ORG_ID").ok(),
...
fn engine_config_from_env() -> harness_engine::EngineConfig {
    // Dev-mode bearer (no CodeGraff): an explicit token enables sync.
    let edge_token = paths::var("HARNESS_EDGE_TOKEN").ok();
    harness_engine::EngineConfig {
        ...
        ipc_port: paths::var("HARNESS_IPC_PORT")
            ...
            .unwrap_or(27664),
        default_harness: harness_from_env(),
        // CodeGraff mode: the signed-in session's org wins; HARNESS_ORG_ID (dev
        // default "dev-org") scopes the workspace room otherwise.
        org_id: paths::var("HARNESS_ORG_ID").ok(),
        // Real auth against production by default; see
        // `codegraff_client_id_from_env` for the dev-mode escape hatches.
        codegraff_client_id: codegraff_client_id_from_env(&edge_token),
        edge_token,
    }
}

/// `HARNESS_PROVIDER` (kebab-case id) picks the default
/// agent for chats without a config row — `mock` powers the e2e smoke; default `graff`.
fn harness_from_env() -> harness_engine::HarnessId {
    match paths::var("HARNESS_PROVIDER").ok().as_deref().map(str::trim) {
...
/// `harness sync`: dial the running engine's IPC and print per-room sync state.
...
async fn sync_cli(ipc_port: u16) -> anyhow::Result<()> {
    let client = harness_rpc::connect_ws(&format!("ws://127.0.0.1:{ipc_port}"))
        .await
        .map_err(|e| {
            anyhow::anyhow!("no engine listening on 127.0.0.1:{ipc_port} ({e}) — is Harness running?")
        })?;
    let status = client
...
/// `{data_dir}/logs/harness-{mode}.log`, previous launch preserved as `.old`.
...
/// locked logs to `harness-{mode}.{pid}.log` instead; the next lock-holding
...
fn open_log_file_in(dir: &std::path::Path, mode: &str) -> Option<std::fs::File> {
    ...
    let path = dir.join(format!("harness-{mode}.log"));
    #[cfg(unix)]
    {
        ...
        if rc != 0 {
            ...
            return std::fs::File::create(
                dir.join(format!("harness-{mode}.{}.log", std::process::id())),
            )
            .ok();
        }
        ...
        drop(existing);
        if preexisting {
            let _ = std::fs::rename(&path, dir.join(format!("harness-{mode}.log.old")));
        }
...
mod log_file_tests {
    ...
    fn second_launch_never_rotates_a_live_processes_log() {
        ...
        assert!(dir.join("harness-headed.log").is_file());
        ...
        let pid_path = dir.join(format!("harness-headed.{}.log", std::process::id()));
        assert!(pid_path.is_file(), "expected pid-suffixed overflow log");
        assert!(
            !dir.join("harness-headed.log.old").exists(),
        ...
        assert!(
            dir.join("harness-headed.log.old").is_file(),
            "rotation resumes"
        );
        drop(third);
    }
...
/// Delete `harness-{mode}.{pid}.log` overflow files older than a week — they
...
fn sweep_stale_pid_logs(dir: &std::path::Path, mode: &str) {
    ...
    let prefix = format!("harness-{mode}.");
