//! `harness login` / `harness logout` / `harness status` — the standalone auth surface.
//!
//! Sign-in used to live only inside `harness headless`, coupling authentication to
//! the long-running daemon. These commands work on the persisted session
//! (`{data_dir}/session.json`) and exit, so a service-managed `harness headless`
//! only ever *loads* credentials. While an engine is running it owns the session
//! (CodeGraff refresh tokens are single-use and rotate on every refresh), so login
    ...
    match scope {
        ...
        WorkspaceScope::Synced => match auth {
            ...
            AuthState::NeedsOrganization { user } => AccountStatus {
                mode: "synced",
                auth: format!(
                    "signed in as {} but no workspace selected — run `harness login`",
                    user.email
                ),
                healthy: false,
            },
            AuthState::SignedOut => AccountStatus {
                mode: "synced",
                auth: "saved session is no longer valid — run `harness login`".into(),
                healthy: false,
            },
        },
    }
...
/// `harness login`: authenticate via the paste-code flow (and workspace
...
pub async fn login(config: EngineConfig) -> anyhow::Result<()> {
    ...
    if !auth.codegraff_enabled() {
        println!("Auth is in dev mode (no CodeGraff client id) — there is nothing to sign in to.");
        return Ok(());
    }
    if let AuthState::SignedIn { user, org_id } = auth.state() {
        ...
        println!("Run `harness logout` first to switch accounts.");
        println!("The next engine start will use the synced workspace.");
        return Ok(());
    }
    if !std::io::stdin().is_terminal() {
        anyhow::bail!("harness login needs an interactive terminal");
    }
...
/// `harness logout`: remove the persisted session.
pub async fn logout(config: EngineConfig) -> anyhow::Result<()> {
    ...
    if !auth.codegraff_enabled() {
        ...
        // previous CodeGraff-mode run so the next real run starts signed out.
        auth.sign_out();
...
/// `harness status`: report the fixed scope a new engine would select, optional
    ...
    InstanceLock::acquire(&config.data_dir).map_err(|err| {
        anyhow::anyhow!(
            "{err}\nCannot {verb} while an engine is running — stop it first \
             (`harness daemon stop`, or quit the Harness app), or use the running UI instead."
        )
    })
}
