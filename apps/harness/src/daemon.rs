//! `harness daemon …` — install/manage `harness headless` as a background service:
...
//! `HARNESS_*` environment captured at install time, so
//! `HARNESS_EDGE_URL=… harness daemon install` bakes that override in.
//!
//! Auth is decoupled: without a saved session the service remains up on the
//! local-only profile. `harness login` and a service restart opt into sync.
...
/// `harness daemon …` manages that installation rather than a competing copy.
const SYSTEMD_UNIT: &str = "harness.service";
...
/// default PATH won't find); the `HARNESS_*`/logging vars only when set.
const CAPTURED_ENV: &[&str] = &[
    ...
    "HARNESS_EDGE_TOKEN",
    "HARNESS_ORG_ID",
    "HARNESS_CODEGRAFF_CLIENT_ID",
    "HARNESS_CODEGRAFF_API_BASE",
    "HARNESS_IPC_PORT",
...
pub fn install(data_dir: &Path) -> anyhow::Result<()> {
    let exe = std::env::current_exe().context("resolving the Harness executable path")?;
    ...
    if cfg!(target_os = "macos") {
        ...
        bail!("Harness daemon is only supported on macOS (launchd) and Linux (systemd)");
    }
    println!(
...
pub fn start() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        let plist = launchd_plist_path()?;
        if !plist.exists() {
            bail!("not installed — run `harness daemon install` first");
        }
        ...
        if !output.status.success() {
            println!(
                ...
                if launchd_plist_path()?.exists() {
                    " (installed — `harness daemon start`)"
                } else {
                    " (not installed — `harness daemon install`)"
                }
            );
            return Ok(());
        }
...
fn render_systemd_unit(exe: &Path, env: &[(String, String)]) -> String {
    let mut unit = String::from(
        "[Unit]\nDescription=Harness headless engine\nAfter=network-online.target\nStartLimitIntervalSec=60\nStartLimitBurst=5\n\n[Service]\n",
    );
...
/// The ExecStart binary path. An exe under `~/.harness/app/` came from the
...
fn exec_path_for(exe: &Path, home: Option<&Path>) -> String {
    let installed = home
        .map(|home| home.join(".harness/app"))
        .is_some_and(|app_root| exe.starts_with(app_root));
    if installed {
        "%h/.harness/app/current/harness".to_string()
    } else {
        format!("{}", exe.display())
    }
}
...
mod tests {
    ...
    fn systemd_unit_shape() {
        let unit = render_systemd_unit(
            Path::new("/usr/local/bin/harness"),
            &[
                ("PATH".into(), "/usr/bin:/bin".into()),
                ("HARNESS_EDGE_URL".into(), "https://edge.example".into()),
                ("RUST_LOG".into(), "info,harness=\"debug\"".into()),
            ],
        );
        assert!(unit.contains("ExecStart=/usr/local/bin/harness headless\n"));
        assert!(unit.contains("Environment=\"PATH=/usr/bin:/bin\"\n"));
        assert!(unit.contains("Environment=\"HARNESS_EDGE_URL=https://edge.example\"\n"));
        // Inner quotes escaped so systemd re-parses the value verbatim.
        assert!(unit.contains("Environment=\"RUST_LOG=info,harness=\\\"debug\\\"\"\n"));
        ...
        assert!(unit.contains("EnvironmentFile=-%h/.harness/env"));
    ...
    fn curl_installer_always_starts_the_local_capable_service() {
        ...
        assert!(installer.contains("systemctl --user enable harness"));
        assert!(installer.contains("systemctl --user restart harness"));
    }
    ...
    fn installed_exe_uses_the_current_symlink() {
        ...
        assert_eq!(
            exec_path_for(
                Path::new("/home/u/.harness/app/0.3.0/harness"),
                Some(Path::new("/home/u")),
            ),
            "%h/.harness/app/current/harness"
        );
        // Source build: literal path.
        assert_eq!(
            exec_path_for(
                Path::new("/src/target/debug/harness"),
                Some(Path::new("/home/u"))
            ),
            "/src/target/debug/harness"
        );
    }
    ...
    fn launchd_plist_shape() {
        let plist = render_launchd_plist(
            Path::new("/Users/x/harness & co/harness"),
            &[("HARNESS_EDGE_URL".into(), "https://e?a=1&b=2".into())],
            Path::new("/Users/x/.harness/daemon.log"),
        );
        assert!(plist.contains("<key>Label</key><string>harness.codegraff.app</string>"));
        // XML-escaped exe path and env value.
        assert!(plist.contains("<string>/Users/x/harness &amp; co/harness</string>"));
        ...
        assert!(
            plist.contains("<key>StandardOutPath</key><string>/Users/x/.harness/daemon.log</string>")
        );
    }
}
