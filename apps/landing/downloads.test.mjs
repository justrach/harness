...
for (const [platform, userAgent, file] of [
  ...
  for (const id of ['nav-download', 'hero-download', 'closing-download']) assert.equal(links[id].href, `https://edge.codegraff.com/releases/harness-0.2.67-${file}`);
  assert.equal(choices.length, 4);
  for (const choice of choices) assert.match(choice.href, /^https:\/\/edge.codegraff.com\/releases\/harness-0.2.67-/);
});
