...
// `harness headless` engine in dev mode) and the app exercises the full live
    ...
    static func runLive(model: AppModel) async {
        ...
        guard let workspace else {
            log("FAIL workspace never connected: store=\(model.workspace != nil) "
                ...
                + "access=\(Keychain.load(key: "codegraffAccessToken") != nil) "
                + "refresh=\(Keychain.load(key: "codegraffRefreshToken") != nil) "
