...
final class AppModel {
    ...
    @ObservationIgnored @AppStorage("harnessAuthMode") var authModeRaw = AppConfig.Mode.codegraff.rawValue
    ...
    func restore() {
        ...
        // Simulator rig: seed CodeGraff tokens straight into the keychain (the
        // ASWebAuthenticationSession flow can't be driven headlessly).
        override("-setaccess") { Keychain.save($0, key: "codegraffAccessToken") }
        override("-setrefresh") { Keychain.save($0, key: "codegraffRefreshToken") }
        ...
        let mode = AppConfig.Mode(rawValue: authModeRaw) ?? .codegraff
        switch mode {
        case .dev:
        ...
        case .codegraff:
            guard let access = Keychain.load(key: "codegraffAccessToken"),
                  let refresh = Keychain.load(key: "codegraffRefreshToken") else { return }
            connect(url: url, mode: .codegraff, userId: storedUserId, orgId: storedOrgId,
    ...
    func signIn(edgeURL: URL, code: String, codeVerifier: String,
                redirectURI: String, nonce: String) async throws {
    ...
    func flushDocs() {
        ...
        let identifier = UIApplication.shared.beginBackgroundTask(withName: "harness.flushDocs") {
    ...
    private func startPathMonitor() {
        ...
        monitor.start(queue: DispatchQueue(label: "harness.path-monitor"))
        pathMonitor = monitor
    }
